...
initial equation
  ...
  ICS20 = V_S2;
equation
  if (V_CU > 0 or V_CU < 0) and not (V_CL > 0 or V_CL < 0) then
    if VCT > V_CL then
      VOTHSG = VSS;
    else
      VOTHSG = 0;
    end if;
  elseif (V_CL > 0 or V_CL < 0) and not (V_CU > 0 or V_CU < 0) then
    if VCT < V_CU then
      VOTHSG = VSS;
    else
      VOTHSG = 0;
    end if;
  elseif (V_CU > 0 or V_CU < 0) and (V_CL > 0 or V_CL < 0) then
    VOTHSG = VSS;
