...
model Components_unpackaged_system_test
  ...
  extends OpenIPSL.Tests.BaseClasses.SMIB(SysData(fn=60), pwFault(
      ...
      t1=62.0,
      t2=62.15));
  BasicComponentsAndCntrlSchemes.VSIO_for_GFM VS(fn=60, M_b=100)
    annotation (Placement(transformation(extent={{-102,-18},{-62,22}})));
  LimitersAndCntrlSchemes.P_f_droop_scheme basic_dROOP(
    f0=60,
    mp=0.01,
  ...
  LimitersAndCntrlSchemes.Q_v_droop_scheme q_v_droop_scheme(
    QVFlag_val_k=true,
    VFlag_val_k=true,
    mq=0.01)
  ...
  Modelica.Blocks.Sources.RealExpression sig_V0(y=VS.vt0) annotation (
      Placement(transformation(
        ...
        rotation=180,
        origin={-198,174})));
  LimitersAndCntrlSchemes.Plimiter plimiter
    annotation (Placement(transformation(extent={{-260,-100},{-220,-140}})));
  BasicComponentsAndCntrlSchemes.low_pass_filter Pfilt
  ...
  LimitersAndCntrlSchemes.Qlimiter qlimiter
  ...
  BasicComponentsAndCntrlSchemes.low_pass_filter Qfilt(y0val=5.416582/100)
